...
HEXAGON_SDK_PATH=/local/mnt/workspace/Qualcomm/Hexagon_SDK/6.2.0.1
...
################# for multimodal test #####################
mtmd_running_params=" -c 4096 --temp 0.7 --top-p 0.8 --top-k 100 --repeat-penalty 1.05 -t 6 "
...
function run_llama_mtmd_cli()
{
    prepare_run_on_phone llama-mtmd-cli

    adb shell "cd ${REMOTE_PATH} \
               && export LD_LIBRARY_PATH=${REMOTE_PATH} \
               && ${REMOTE_PATH}/llama-mtmd-cli -m ${GGUF_MODEL_NAME} --mmproj ${MMPROJ_MODEL_PATH} ${mtmd_running_params} --image ${IMG_PATH} -p \"What is in the image?\n\""
...
if [ $# == 0 ]; then
    ...
    if [ "$1" == "-h" ]; then
        ...
        update_qnn_libs
        exit 0
    elif [ "$1" == "run_llama_mtmd_cli" ]; then
        run_llama_mtmd_cli
        exit 0
    else
        show_usage
